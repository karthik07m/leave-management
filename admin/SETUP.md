...
## Pre Nov-20th
  ...
  * Create  repository for the 10 teams
    ...
    * `git clone --bare ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp93`
    * 'cd ftp93`
    ...
    * For each team, nn = 01 to 10
      ...
      * For windows
        ...
        * cd ftpnn
        * use VS Code, find/replace in files to do the following: replacing ftp93 -> ftpnn, ftp93 -> FTPnn
        * Rename directory restservice/leavemanager/src/main/java/com/hexaware/ftp93/ to restservice/leavemanager/src/main/java/com/hexaware/ftpnn/
      * For Mac OS; replace nn with the team-id in the following three commands; use xxnn.sh
        * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTPnn.git`
        * `cd ftpnn`      
        * `find . -type f -not -path '*/\.*' -not -path "*/node_modules/*" -exec sed -i '' -e 's/ftp93/ftp[nn]/g' {} \; -exec sed -i '' -e 's/ftp93/FTP[nn]/g' {} \;`
        * `mv restservice/leavemanager/src/main/java/com/hexaware/ftp93/ restservice/leavemanager/src/main/java/com/hexaware/ftp[nn]/`
        * `mv restservice/leavemanager/src/test/java/com/hexaware/ftp93/ restservice/leavemanager/src/test/java/com/hexaware/ftp[nn]/`
        * `mv restservice/integration-test/src/test/java/com/hexaware/ftp93 restservice/integration-test/src/test/java/com/hexaware/ftp[nn]`
  ...
  * Databases - 3 per team
    * Integration and Staging instances
      ...
      * 
    * `CREATE DATABASE ftp93;` -- XX from 01 to 10
    * `CREATE USER 'ftp93'@'%' IDENTIFIED BY 'ftp93';`
    * `GRANT ALL ON ftp93.* TO 'ftp93'@'%';`
    * `select Host, User, Password from mysql.user order by user;`
    * `select * from mysql.db order by Db;`
  * Jenkins jobs - 3 per team
    * ftp93-10-UNIT
    * ftp93-30-INTEGRATION
    * ftp93-50-STAGING
    * For each team, nn = 01 to 10
      * Create a tab FTPnn
...
Create a "ftp93" as project board and add users through add members link at Trello board
